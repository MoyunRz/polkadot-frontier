	...
	BE: Backend<B> + 'static,
{
	...
	pub async fn fee_history_task(
		...
		#[rustfmt::skip]
			...
			log::info!("=======> fee history task base fee <=======");
			log::info!("{:?}",base_fee.clone());
			...
			if let (Some(block), Some(receipts)) = (block, receipts) {
				...
				log::info!("=======> fee history task transactions <=======");
				log::info!("{:?}",transactions.len());
				log::info!("{:?}",receipts.clone());
			...
			// result.rewards.push(500);
			log::info!("=======> fee history task <=======");
			log::info!("{:?},{:?}",result.base_fee,result.rewards);
			(result, block_number)
		};
		...
		let commit_if_any = |item: FeeHistoryCacheItem, key: Option<u64>| {
				...
				(key, &mut fee_history_cache.lock())
			{
				...
				log::info!("=======> fee history task block limit<=======");
				log::info!("{:?},{:?}",block_number.clone(),block_limit.clone());
