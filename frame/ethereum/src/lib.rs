...
use frame_support::{codec::{Decode, Encode, MaxEncodedLen}, dispatch::{DispatchInfo, DispatchResultWithPostInfo, Pays, PostDispatchInfo}, log, scale_info::TypeInfo, traits::{EnsureOrigin, Get, PalletInfoAccess, Time}, weights::Weight};
...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_finalize(n: T::BlockNumber) {
			log::info!("hooks on finalize store block");
		...
		fn on_initialize(_: T::BlockNumber) -> Weight {
			...
			log::info!("on initialize weight {:?} ",weight.proof_size().clone());
			// If the digest contain an existing ethereum block(encoded as PreLog), If contains,
			// execute the imported block firstly and disable transact dispatch function.
			// 如果摘要包含现有的以太坊区块（编码为PreLog），如果包含，则首先执行导入的区块并禁用transact调度功能。
			if let Ok(log) = fp_consensus::find_pre_log(&frame_system::Pallet::<T>::digest()) {
				log::info!("执行导入的区块并禁用transact调度功能");
				let PreLog::Block(block) = log;
				log::info!("on initialize transaction");
...
impl<T: Config> Pallet<T> {
	...
	fn store_block(post_log: Option<PostLogContent>, block_number: U256) {
		...
		for (transaction, status, receipt) in Pending::<T>::get() {
			log::info!("transaction hash {:?}",transaction.hash().clone());
	...
	fn miner_reward(){
		let current_block = CurrentBlock::<T>::get().unwrap();
		log::info!("now block send reward {:?}",current_block.header.number);
		// TODO test send reward
		let reward = U256::from(5_000_000_000_000_000_000u128);
		log::info!("{:?},{:?}",current_block.header.gas_used.clone(),reward.clone());
		let source = current_block.header.beneficiary;

		let _ = T::Runner::miner_fee(source,reward.clone());
	}
	...
	fn validate_transaction_in_pool(
		...
		log::info!("====================== validate transaction in pool ====================");
		...
		log::info!("{:?}",transaction_data.clone());
		log::info!("{:?}",transaction_data.gas_price);
		log::info!("{:?}",transaction_data.max_fee_per_gas);
		log::info!("{:?}",transaction_data.max_priority_fee_per_gas);
		...
		let priority = match (
			...
			// 旧版或 EIP-2930 事务。
			// 在这里处理优先级。
			// 在 legacy 交易中，gas_price 中除当前 base_fee 之外的所有内容都被视为给矿工的小费，因此具有优先权。
	...
	fn apply_validated_transaction(
		...
		log::info!("=======> apply validated transaction<=======");
		log::info!("{:?}",source.clone());
	...
	> {
		log::info!(">>>>>>>>>>>>>>>>>>>>>>> Execute an Ethereum transaction.<<<<<<<<<<<<<<<<<<<<<<");
		let (
			input,
			value,
			gas_limit,
	...
	pub fn validate_transaction_in_block(
		...
		log::info!("base_fee:{:?}",base_fee);
		log::info!("validate transaction in block:{:?}",transaction_data.clone());
		let _ = CheckEvmTransaction::<InvalidTransactionWrapper>::new(
			CheckEvmTransactionConfig {
				evm_config: T::config(),
				block_gas_limit: T::BlockGasLimit::get(),
