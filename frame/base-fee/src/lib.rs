...
pub mod pallet {
	...
	impl<T: Config> Hooks<BlockNumberFor<T>> for Pallet<T> {
		fn on_initialize(_: T::BlockNumber) -> Weight {
			...
			frame_support::log::info!("------------------- base fee on initialize ------------------");
		...
		fn on_finalize(_n: <T as frame_system::Config>::BlockNumber) {
			if <Elasticity<T>>::get().is_zero() {
				...
				frame_support::log::info!("------------------ Zero elasticity means constant BaseFeePerGas. ------------------");
				return;
			}
			frame_support::log::info!("------------------ base fee on finalize ------------------");
