...
use sp_runtime::{traits::{BadOrigin, Saturating, UniqueSaturatedInto, Zero}, AccountId32, DispatchErrorWithPostInfo, DispatchError};
		...
		U256: UniqueSaturatedInto<BalanceOf<T>>,
	{
		fn build(&self) {
			...
			for (address, account) in &self.accounts {
				...
				log::info!("evm build create account {:?},{:?},{:?}",account_id.clone(),address.clone(),account.code.clone());
				...
				for (index, value) in &account.storage {
					log::info!("evm build storage account {:?},{:?},{:?}",address.clone(),index.clone(),value.clone());
...
pub trait OnChargeEVMTransaction<T: Config> {
	...
	/// Introduced in EIP1559 to handle the priority tip.
	fn miner_fee(who: &H160, reward: U256);
}
	...
	U256: UniqueSaturatedInto<<C as Currency<<T as frame_system::Config>::AccountId>>::Balance>,
{
	...
	fn pay_priority_fee(tip: Self::LiquidityInfo) {
		log::info!("Default Ethereum behaviour: issue the tip to the block author.");
		// Default Ethereum behaviour: issue the tip to the block author.
		if let Some(tip) = tip {
			let account_id = T::AddressMapping::into_account_id(<Pallet<T>>::find_author());
			log::info!("account_id:{:?},peek:{:?}",account_id.clone(),tip.peek());
			// 如果 who 不存在，则不执行任何操作并返回 Err
			// let res = C::deposit_into_existing(&account_id, tip.peek());
			// 为 who 的自由余额增加价值。如果谁不存在，那么它就会被创建。绝对正确
			C::deposit_creating(&account_id, tip.peek());
		}
	}

	fn miner_fee(who: &H160, reward: U256) {
		log::info!("Default Ethereum behaviour: miner fee");
		// Default Ethereum behaviour: issue the tip to the block author.
		let account_id = T::AddressMapping::into_account_id(<Pallet<T>>::find_author());
		let res = C::deposit_creating(&account_id, reward.unique_saturated_into());
		let free = C::free_balance(&account_id);
		log::info!("{:?},{:?}",res.peek(),free);
	}
}
...

{
	...
	fn miner_fee(who: &H160, reward: U256) {
		<EVMCurrencyAdapter::<<T as Config>::Currency, ()> as OnChargeEVMTransaction<T>>::miner_fee(who,reward);
	}
}
